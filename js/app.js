(() => {
    ...
    da.init();
    const simplicity = document.querySelector(".order__marquee");
    if (simplicity) {
        const banerStart = document.querySelector(".order__title");
        let banerText = banerStart.innerHTML;
        const banerSpanStart = `<div class="order__title">${banerText}</div>`;
        function animationBanerText() {
            ...
            if (spanQuantity > 1) banerWidth = spanQuantity * textWidth + textWidth + 50; else banerWidth = (spanQuantity + 1) * textWidth + textWidth;
            simplicity.style.width = banerWidth + "px";
            for (let i = 0; i < spanQuantity; i++) banerStart.insertAdjacentHTML("afterEnd", banerSpanStart);
            document.querySelectorAll(".order__title").forEach((item => {
                item.animate([ {
